...
model = load_model()  # Load model every time


# Image Preprocessing & Prediction Function
def model_prediction(test_image):
    image = Image.open(test_image).convert("RGB")  # Ensure it's RGB
    ...
    if test_image is not None:
        ...
        if st.button("Predict"):
            with st.spinner("Analyzing..."):
                ...
                st.write(f"🔍 Confidence Score: {np.max(model.predict(input_arr)):.2f}")
